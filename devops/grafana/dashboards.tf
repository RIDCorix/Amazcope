...
# ============================================================================
# cAdvisor Dashboard - Container Metrics from Prometheus
# ============================================================================
...
resource "grafana_dashboard" "cadvisor" {
  ...
  config_json = jsonencode({
    ...
    tags        = ["prometheus", "cadvisor", "containers", "docker"]
    timezone    = "browser"
    refresh     = "30s"
    time = {
      from = "now-1h"
    ...
    panels = [
      # CPU Usage per Container
      {
        ...
        gridPos   = { x = 0, y = 0, w = 12, h = 8 }
        targets = [{
          expr = "rate(container_cpu_usage_seconds_total{name=~\"$container\", instance=~\"$instance\"}[5m]) * 100"
          legendFormat = "{{name}}"
          refId = "A"
        }]
        yaxes = [{
      ...
      # Memory Usage per Container
      {
        ...
        gridPos   = { x = 12, y = 0, w = 12, h = 8 }
        targets = [{
          expr = "container_memory_usage_bytes{name=~\"$container\", instance=~\"$instance\"}"
          legendFormat = "{{name}}"
          refId = "A"
        }]
        yaxes = [{
        ...
        alert = {
          conditions = [{
            ...
            evaluator = {
              type   = "gt"
              params = [1073741824] # Alert if memory > 1GB
            }
          }]
          executionErrorState = "alerting"
          frequency           = "60s"
      ...
      # Container Network Traffic
      {
        ...
        targets = [
          {
            expr = "rate(container_network_receive_bytes_total{name=~\"$container\", instance=~\"$instance\"}[5m])"
            legendFormat = "RX - {{name}}"
            refId = "A"
          },
          {
            expr = "rate(container_network_transmit_bytes_total{name=~\"$container\", instance=~\"$instance\"}[5m])"
      ...
      # Container Filesystem Usage
      {
        ...
        targets = [
          {
            expr = "rate(container_fs_reads_bytes_total{name=~\"$container\", instance=~\"$instance\"}[5m])"
            legendFormat = "Read - {{name}}"
            refId = "A"
          },
          {
            expr = "rate(container_fs_writes_bytes_total{name=~\"$container\", instance=~\"$instance\"}[5m])"
      ...
      # Container Count
      {
        ...
        targets = [{
          expr = "count(container_last_seen{instance=~\"$instance\", name=~\"$container\"})"
      ...
      # Container Restarts
      {
        ...
        targets = [{
          expr = "changes(container_last_seen{instance=~\"$instance\", name=~\"$container\"}[1h])"
          legendFormat = "{{name}}"
          refId = "A"
        }]
        options = {
      ...
      # Memory Limit per Container
      {
        ...
        targets = [{
          expr = "container_spec_memory_limit_bytes{instance=~\"$instance\", name=~\"$container\"}"
          legendFormat = "{{name}}"
          refId = "A"
        }]
        options = {
...
resource "grafana_dashboard" "sentry" {
  count = var.enable_sentry && var.enable_sentry_dashboard ? 1 : 0
  ...
  config_json = jsonencode({
    ...
    tags        = ["sentry", "errors", "apm", "monitoring"]
    timezone    = "browser"
    refresh     = "1m"
    time = {
      from = "now-24h"
      ...
      list = [
        {
          name       = "project"
          type       = "query"
          datasource = try(grafana_data_source.sentry[0].uid, "Sentry")
          query      = "projects"
          refresh    = 1
          multi      = true
          includeAll = true
    ...
    panels = [
      ...
      {
        id        = 4
        title     = "Top 10 Errors by Frequency"
        type      = "table"
        datasource = try(grafana_data_source.sentry[0].uid, "Sentry")
        gridPos   = { x = 0, y = 8, w = 12, h = 8 }
        targets = [{
